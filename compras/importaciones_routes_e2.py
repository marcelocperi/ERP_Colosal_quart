...
def register_importaciones_routes_e2(bp):
    ...
    @login_required
    def importacion_orden_detalle(orden_id):
        ...
        cargos       = ImportacionService.get_cargos_orden(orden_id, ent_id)
        despacho     = ImportacionService.get_despacho(orden_id, ent_id)
        items_orden  = ImportacionService.get_items_orden(orden_id, ent_id)
        ...
        desvios = ImportacionService.get_desvio_costos(orden_id, ent_id)

        # Fase 1: Tracking de Buque
        from services.vessel_tracking_service import VesselTrackingService
        vessel_track = VesselTrackingService.get_last_tracking(orden_id, ent_id)

        return render_template('compras/importacion_orden_detalle.html',
                               orden=orden,
                               cargos=cargos,
                               despacho=despacho,
    ...
    @login_required
    def importacion_agregar_cargo(orden_id):
        """Registra un cargo de importación (flete, derechos, despachante, etc.)."""
        from services.bcra_service import CurrencyRateService
        ent_id = g.user['enterprise_id']

        tipo_cargo   = request.form.get('tipo_cargo', 'OTRO')
        descripcion  = request.form.get('descripcion', tipo_cargo)
        monto_orig   = float(request.form.get('monto_orig', 0) or 0)
        moneda_orig  = request.form.get('moneda_orig', 'ARS').upper()
        proveedor_id = request.form.get('proveedor_id') or None
        fecha        = request.form.get('fecha') or None
        aplica_cui   = 1 if request.form.get('aplica_a_cui') == '1' else 0
        es_estimado  = 1 if request.form.get('es_estimado') == '1' else 0
        ref_id       = request.form.get('cargo_referencia_id') or None

        # Tipo de cambio
        from services.bcra_service import CurrencyRateService
        tc_manual = float(request.form.get('tipo_cambio', 0) or 0)
        if tc_manual <= 0 and moneda_orig != 'ARS':
            tc_manual = CurrencyRateService.get_tipo_cambio(moneda_orig) or 1000.0
        elif moneda_orig == 'ARS':
            tc_manual = 1.0

        if monto_orig <= 0:
            flash("El monto del cargo debe ser mayor a cero.", "danger")
            return redirect(url_for('compras.importacion_orden_detalle', orden_id=orden_id))

        try:
            ImportacionService.agregar_cargo(
            ...
            flash(f"Cargo '{descripcion}' registrado correctamente.", "success")
        except Exception as e:
            flash(f"Error al registrar cargo: {str(e)}", "danger")

        return redirect(url_for('compras.importacion_orden_detalle', orden_id=orden_id))

    @bp.route('/compras/importaciones/cargo/<int:cargo_id>/eliminar', methods=['POST'])
    @login_required
    def importacion_eliminar_cargo(cargo_id):
    ...
    def api_calcular_cui(orden_id):
        ...
        try:
            resultado = ImportacionService.calcular_cui(
    ...
    @login_required
    def importacion_guardar_despacho(orden_id):
        """Crea o actualiza el despacho aduanero de una OC de importación."""
        ent_id = g.user['enterprise_id']
        data = {
            'numero_despacho':            request.form.get('numero_despacho', ''),
            'despachante_id':             request.form.get('despachante_id') or None,
            'fecha_oficializacion':       request.form.get('fecha_oficializacion') or None,
            'canal':                      request.form.get('canal', 'VERDE'),
            'estado':                     request.form.get('estado', 'PENDIENTE'),
            'valor_fob_usd':              request.form.get('valor_fob_usd', 0),
            'valor_cif_usd':              request.form.get('valor_cif_usd', 0),
            'derechos_ars':               request.form.get('derechos_ars', 0),
            'tasa_estadistica_ars':       request.form.get('tasa_estadistica_ars', 0),
            'otros_tributos_ars':         request.form.get('otros_tributos_ars', 0),
            'tipo_cambio_oficializacion': request.form.get('tipo_cambio_oficializacion', 0),
            'observaciones':              request.form.get('observaciones', ''),
            # Etapa 4: Logística
            'transportista':              request.form.get('transportista'),
            'guia_bl_tracking':           request.form.get('guia_bl_tracking'),
            'fecha_embarque':             request.form.get('fecha_embarque') or None,
            'fecha_arribo_estimada':      request.form.get('fecha_arribo_estimada') or None,
            'fecha_arribo_real':          request.form.get('fecha_arribo_real') or None,
            'puerto_embarque':            request.form.get('puerto_embarque'),
            'puerto_destino':             request.form.get('puerto_destino'),
            'bultos':                     request.form.get('bultos', 0),
            'peso_kg':                    request.form.get('peso_kg', 0),
        }

        try:
            despacho_id = ImportacionService.crear_o_actualizar_despacho(
                ent_id, orden_id, data, g.user['id']
            )
            flash(f"Despacho #{data['numero_despacho'] or despacho_id} guardado correctamente.", "success")
        except Exception as e:
            flash(f"Error al guardar despacho: {str(e)}", "danger")
    ...
    def importacion_ingresar_stock(orden_id):
        """
        ...
        tc_manual   = float(request.form.get('tipo_cambio', 0) or 0) or None

        if not deposito_id:
            flash("Debe seleccionar el depósito de destino.", "danger")
            return redirect(url_for('compras.importacion_orden_detalle', orden_id=orden_id))

        # Obtener despacho activo
        despacho = ImportacionService.get_despacho(orden_id, ent_id)
        if not despacho:
            flash("No existe un Despacho Aduanero registrado para esta orden.", "danger")
            return redirect(url_for('compras.importacion_orden_detalle', orden_id=orden_id))

        if despacho.get('estado') not in ('LIBERADO', 'INGRESADO'):
            flash(f"El despacho debe estar en estado LIBERADO para ingresar al stock. "
                  f"Estado actual: {despacho.get('estado')}", "warning")
            return redirect(url_for('compras.importacion_orden_detalle', orden_id=orden_id))

        if despacho.get('estado') == 'INGRESADO':
            flash("Esta importación ya fue ingresada al stock anteriormente.", "warning")
            return redirect(url_for('compras.importacion_orden_detalle', orden_id=orden_id))

        try:
            resultado = ImportacionService.registrar_ingreso_stock(
            ...
            if resultado['success']:
                ...
                flash(f"Error en el ingreso: {resultado.get('message')}", "danger")

        except Exception as e:
            flash(f"Error inesperado al ingresar importación: {str(e)}", "danger")
    ...
    @login_required
    def api_liberar_despacho(despacho_id):
        ...
        try:
            with get_db_cursor() as cursor:
                ...
                if cursor.rowcount == 0:
                    return jsonify({'success': False, 'message': 'Despacho no encontrado'}), 404
            ...
            return jsonify({'success': True, 'message': 'Despacho liberado correctamente.'})
        except Exception as e:
            return jsonify({'success': False, 'message': str(e)}), 500
    ...
    @login_required
    def api_resumen_importacion(orden_id):
        """Retorna resumen de costos + CUI para la OC de importación."""
        ent_id = g.user['enterprise_id']
        try:
            cargos   = ImportacionService.get_cargos_orden(orden_id, ent_id)
            despacho = ImportacionService.get_despacho(orden_id, ent_id)
            cui      = ImportacionService.calcular_cui(orden_id, ent_id)
    ...
    @login_required
    def importaciones_dashboard_avanzado():
        """Dashboard Ejecutivo de Importaciones (Stage 5)."""
        ent_id = g.user['enterprise_id']
        stats = ImportacionService.get_dashboard_stats(ent_id)
        
        return render_template('compras/importaciones_dashboard_avanzado.html', 
                               stats=stats)

    @bp.route('/compras/api/importaciones/orden/<int:orden_id>/desvios')
    @login_required
    def api_desviacion_costos(orden_id):
        """API para análisis de desvíos entre estimado y real."""
        ent_id = g.user['enterprise_id']
        try:
            stats = ImportacionService.get_desvio_costos(orden_id, ent_id)
    ...
    @login_required
    def api_track_vessel(orden_id):
        """API para rastrear buque AIS."""
        from services.vessel_tracking_service import VesselTrackingService
        ent_id = g.user['enterprise_id']
        mmsi = request.json.get('mmsi')
        ...
        try:
            ...
            res = VesselTrackingService.track_vessel_by_mmsi(ent_id, orden_id, mmsi, g.user['id'])
            return jsonify(res)
        except Exception as e:
            return jsonify({'success': False, 'message': str(e)}), 500
