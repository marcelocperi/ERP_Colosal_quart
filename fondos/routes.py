...
def dashboard():
    try:
        return render_template('fondos/dashboard.html')
    except Exception as e:
        import traceback
        traceback.print_exc()
        flash(f"Error al cargar el dashboard de fondos: {str(e)}", "danger")
        return redirect('/')
...
def cajas():
    with get_db_cursor(dictionary=True) as cursor:
        cursor.execute("SELECT * FROM erp_cuentas_fondos WHERE enterprise_id = %s", (g.user['enterprise_id'],))
        cajas = cursor.fetchall()
    return render_template('fondos/cajas.html', cajas=cajas)
...
def medios_pago():
    with get_db_cursor(dictionary=True) as cursor:
        if request.method == 'POST':
            try:
                ...
                if action == 'create':
                    ...
                    cuenta_contable_id = request.form.get('cuenta_contable_id') or None
                    recargo = request.form.get('recargo_pct', 0)
                    ...
                    flash('Medio de pago creado exitosamente', 'success')
                    ...
                    flash('Medio de pago actualizado', 'success')
                
                elif action == 'delete':
                    mid = request.form['id']
                    cursor.execute("DELETE FROM fin_medios_pago WHERE id=%s AND enterprise_id=%s", (mid, g.user['enterprise_id']))
                    flash('Medio de pago eliminado', 'warning')
        ...
        # GET
        cursor.execute("SELECT * FROM fin_medios_pago WHERE (enterprise_id = %s OR enterprise_id = 0) ORDER BY nombre", (g.user['enterprise_id'],))
        ...
        cursor.execute("SELECT id, codigo, nombre FROM cont_plan_cuentas WHERE tipo = 'ACTIVO' AND enterprise_id = %s ORDER BY codigo", (g.user['enterprise_id'],))
        cuentas = cursor.fetchall()

    return render_template('fondos/medios_pago.html', medios=medios, cuentas=cuentas)
...
def aprobaciones():
    ...
    return render_template('fondos/aprobaciones_po.html', ordenes=ordenes)
...
def aprobar_po_detalle(id):
    ...
    return render_template('fondos/aprobar_po_detalle.html', po=po, items=items)
...
def post_aprobacion_po(id):
    ...
    action = request.form.get('action')
    fecha_pago = request.form.get('fecha_pago_estimada')
    ...
    with get_db_cursor(dictionary=True) as cursor:
        if action == 'approve':
            ...
            flash(f"Fondos aprobados para PO #{id}. Pago estimado: {fecha_pago}", "success")
            ...
            flash(f"PO #{id} enviada de regreso a Compras para ajuste.", "warning")
...
def kpi_desembolsos():
    ...
    return render_template('fondos/kpi_desembolsos.html', calendario=calendario)
...
def condiciones_pago():
    with get_db_cursor(dictionary=True) as cursor:
        if request.method == 'POST':
            try:
                ...
                if action == 'create':
                    nombre = request.form['nombre']
                    dias = request.form.get('dias_vencimiento', 0)
                    descuento = request.form.get('descuento_pct', 0)
                    recargo = request.form.get('recargo_pct', 0)
                    ...
                    flash('Condición de pago creada', 'success')
                    ...
                    cid = request.form['id']
                    cursor.execute("DELETE FROM fin_condiciones_pago WHERE id=%s AND enterprise_id=%s", (cid, g.user['enterprise_id']))
                    flash('Condición de pago eliminada', 'warning')
        ...
        # GET
        cursor.execute("SELECT * FROM fin_condiciones_pago WHERE (enterprise_id = %s OR enterprise_id = 0) ORDER BY dias_vencimiento, nombre", (g.user['enterprise_id'],))
    ...
    return render_template('fondos/condiciones_pago.html', condiciones=condiciones)

@fondos_bp.route('/fondos/condiciones-mixtas', methods=['GET', 'POST'])
@login_required
@atomic_transaction('fondos', severity=5, impact_category='Operational')
def condiciones_mixtas():
    with get_db_cursor(dictionary=True) as cursor:
        if request.method == 'POST':
            try:
                action = request.form.get('action')
                if action == 'create' or action == 'edit':
                    ...
                    condiciones_ids = request.form.getlist('condicion_pago_id[]')
                    porcentajes = request.form.getlist('porcentaje[]')
                    descuentos = request.form.getlist('descuento_pct_det[]')
                    recargos = request.form.getlist('recargo_pct_det[]')
                    activo = 1 if 'activo' in request.form else 0
                    ...
                    if action == 'create':
                        ...
                        cursor.execute("DELETE FROM fin_condiciones_pago_mixtas_detalle WHERE mixta_id = %s AND enterprise_id = %s", (mixta_id, g.user['enterprise_id']))
                    ...
                    flash('Condición mixta guardada correctamente', 'success')
                    
                elif action == 'delete':
                    mid = request.form['id']
                    cursor.execute("DELETE FROM fin_condiciones_pago_mixtas WHERE id=%s AND enterprise_id=%s", (mid, g.user['enterprise_id']))
                    flash('Condición mixta eliminada', 'warning')
        ...
        # GET
        # Obtener cabeceras
        cursor.execute("SELECT * FROM fin_condiciones_pago_mixtas WHERE (enterprise_id = %s OR enterprise_id = 0) ORDER BY nombre", (g.user['enterprise_id'],))
        mixtas = cursor.fetchall()
        ...
        cursor.execute("SELECT * FROM fin_condiciones_pago WHERE (enterprise_id = %s OR enterprise_id = 0) AND activo = 1 ORDER BY nombre", (g.user['enterprise_id'],))
        condiciones_simples = cursor.fetchall()

    return render_template('fondos/condiciones_mixtas.html', mixtas=mixtas, condiciones_simples=condiciones_simples)
...
def impuestos():
    with get_db_cursor(dictionary=True) as cursor:
            ...
            try:
                ...
                if action == 'create':
                    ...
                    try:
                        ...
                        sync_new_concept_to_all_enterprises('IMPUESTO', nombre)
                    except Exception as e:
                        pass
                    ...
                    iid = request.form['id']
                    cursor.execute("DELETE FROM sys_impuestos WHERE id=%s AND enterprise_id=%s", (iid, g.user['enterprise_id']))
                    flash('Impuesto eliminado', 'warning')
        ...
        # GET
        cursor.execute("SELECT * FROM sys_impuestos WHERE enterprise_id = %s OR enterprise_id = 0 ORDER BY nombre", (g.user['enterprise_id'],))
    ...
    return render_template('fondos/impuestos.html', impuestos=impuestos_lista)
...
def configuracion_global():
    ...
    with get_db_cursor(dictionary=True) as cursor:
        ...
        cursor.execute("SELECT * FROM sys_impuestos WHERE (enterprise_id = %s OR enterprise_id = 0) AND activo = 1 ORDER BY nombre", (g.user['enterprise_id'],))
        impuestos_maestros = cursor.fetchall()
        for imp in impuestos_maestros:
            if imp.get('created_at'): imp['created_at'] = str(imp['created_at'])
            if imp.get('updated_at'): imp['updated_at'] = str(imp['updated_at'])
        
    return render_template('fondos/configuracion_global.html', medios=medios, condiciones=condiciones, mixtas=mixtas, impuestos_maestros=impuestos_maestros)
        ...
        if request.method == 'POST':
            ...
            try:
                if action == 'create':
                    ...
                    tipo_entidad  = request.form.get('tipo_entidad', 'CBU').strip()
                    ...
                    cuenta_id = BCRAService.crear_cuenta_para_banco(
                    ...
                    flash(f'Entidad "{nombre}" creada correctamente.{extra}', 'success')
                    ...
                    if row:
                        if row['enterprise_id'] == 0 or row['origen'] == 'BCRA':
                            ...
                            flash('Banco desactivado (dato maestro BCRA).', 'warning')
                        else:
                            cursor.execute(
                                "DELETE FROM fin_bancos WHERE id=%s AND enterprise_id=%s", (bid, eid))
                            flash('Banco eliminado.', 'warning')
...
def bancos_sincronizar_bcra():
    ...
    try:
        ...
        flash(f'Sin conexión con el BCRA: {e}', 'danger')
    except TimeoutError as e:
        flash(f'Timeout al conectar con el BCRA: {e}', 'danger')
    except Exception as e:
        flash(f'Error al sincronizar bancos: {str(e)}', 'danger')
    return redirect(url_for('fondos.bancos'))


@fondos_bp.route('/fondos/bancos/sincronizar-billeteras', methods=['POST'])
@login_required
def bancos_sincronizar_billeteras():
    ...
    try:
        ...
        flash(f'Error al sincronizar billeteras: {str(e)}', 'danger')
    return redirect(url_for('fondos.bancos'))


@fondos_bp.route('/fondos/bancos/api/buscar')
@login_required
def bancos_api_buscar():
    """
    Endpoint JSON para búsqueda de bancos (usado por select2 / autocomplete).
