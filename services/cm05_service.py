...
class CM05Service:
    ...
    def upsert_coeficiente(enterprise_id, tercero_id, jurisdiccion, periodo_anio, coeficiente, user_id):
        with get_db_cursor(dictionary=True) as cursor:
            # Check if exists
            cursor.execute("SELECT id, user_insert, date_insert, user_update, date_update, coeficiente FROM erp_terceros_cm05 WHERE tercero_id = %s AND enterprise_id = %s AND jurisdiccion_code = %s AND periodo_anio = %s", (tercero_id, enterprise_id, jurisdiccion, periodo_anio))
