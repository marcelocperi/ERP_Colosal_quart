...
class AFIPRejected(Exception):
    def __init__(self, message, codes=None):
...
@ventas_bp.route('/api/ventas/cliente/<int:id>/check-apoc', methods=['POST'])
@login_required
async def api_cliente_check_apoc(id):
    """
    Consulta AFIP para verificar si el cliente figura como contribuyente apócrifo.
    """
    ...
    from services.afip_service import AfipService
    res = await AfipService.consultar_base_apoc(ent_id, cliente['cuit'])
...
async def procesar_factura():
    ...
    try:
        async with get_db_cursor(dictionary=True) as cursor:
            ...
            if not afip_res.get('success'):
                # SI AFIP RECHAZA: Lanzamos excepción personalizada para control total
                error_msg = afip_res.get('error', 'Error desconocido en AFIP')
                raise AFIPRejected(f"AFIP Rechazó la Operación: {error_msg}", codes=afip_res.get('codes', []))
    ...
    except AFIPRejected as e:
        # Error específico de AFIP: devolvemos códigos para el modal premium
        return jsonify({'success': False, 'message': str(e), 'codes': e.codes}), 400
    except Exception as e:
        import traceback
        traceback.print_exc()
